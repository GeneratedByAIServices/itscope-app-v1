...
서비스에 처음 방문한 사용자가 이메일 주소를 사용하여 새로운 계정을 생성하는 과정입니다. **모든 UI 텍스트와 알림 메시지는 현재 설정된 언어(한국어/영어)에 따라 동적으로 표시됩니다.**
...
-   **3단계: 이메일 인증 코드 확인 (Signup Step)**
    1.  회원가입 버튼 클릭 후, 입력한 이메일로 6자리 인증 코드가 발송됩니다.
    2.  인증 코드 입력 UI가 나타나고, 사용자는 이메일로 받은 코드를 입력합니다. 입력 필드는 자동으로 포커싱됩니다.

-   **4단계: 가입 성공 (Success Step)**
    1.  인증이 성공적으로 완료되면, `SuccessStep` 컴포넌트가 렌더링됩니다.
    2.  "성공적으로 가입되었습니다"와 같은 환영 메시지와 함께 사용자의 이메일 주소가 표시됩니다.
    3.  사용자는 '계속' 버튼을 클릭하여 2단계 인증 화면으로 이동합니다.

### 시나리오 2: Google 계정으로 로그인

Google 계정을 이용하여 서비스에 간편하게 로그인하거나 신규 가입하는 과정입니다. **모든 UI 텍스트와 알림 메시지는 현재 설정된 언어(한국어/영어)에 따라 동적으로 표시됩니다.**
...
이미 계정을 가지고 있는 사용자가 이메일과 비밀번호로 서비스에 로그인하는 과정입니다. **모든 UI 텍스트와 알림 메시지는 현재 설정된 언어(한국어/영어)에 따라 동적으로 표시됩니다.**
...
사용자가 서비스에 접속하여 시스템의 주요 공지들을 확인하고 관리하는 과정입니다. 이 흐름은 사용자 인증 흐름과 별개로 초기 화면에서 즉시 상호작용이 가능합니다. **모든 UI 텍스트는 현재 설정된 언어(한국어/영어)에 따라 동적으로 표시됩니다.**
...
-   **2단계: 언어 변경**
    1.  확장된 메뉴 최상단에는 언어 토글 스위치(KOR/ENG)가 있습니다.
    2.  사용자가 이 스위치를 클릭하면, 애플리케이션 전체의 언어가 즉시 한국어 또는 영어로 전환됩니다. 변경된 언어 설정은 브라우저에 저장됩니다.

-   **3단계: 메뉴 기능 사용**
    1.  사용자가 확장된 메뉴(`고객센터 문의`, `자주 묻는 질문` 등) 중 하나를 클릭합니다.
    2.  현재는 개발 중인 기능이므로, "'해당 기능'은 현재 준비중입니다." 라는 안내 메시지가 현재 설정된 언어로 화면에 잠시 나타났다가 사라집니다.

-   **4단계: 공지 상태 초기화 (숨겨진 기능)**
